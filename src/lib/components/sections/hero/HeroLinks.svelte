<script>
	...
	const icons = {
		...
		twitch: `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M2.149 0l-1.612 4.119v16.836h5.731v3.045h3.224l3.045-3.045h4.657l6.269-6.269v-14.686h-21.314zm19.164 13.612l-3.582 3.582h-5.731l-3.045 3.045v-3.045h-4.836v-15.045h17.194v11.463zm-3.582-7.343v6.262h-2.149v-6.262h2.149zm-5.731 0v6.262h-2.149v-6.262h2.149z"/></svg>`,
		...
		website: `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/></svg>`
	};
...
<section class="hero flex min-h-[70vh] items-center justify-center">
	<div class="hero-content flex-col text-center">
		{#if data.showAvatar && data.avatar}
		...
		{/if}
		<h1
			class="bg-linear-to-r from-primary to-secondary bg-clip-text text-5xl font-bold text-transparent md:text-6xl"
		>
			{data.name}
		</h1>
		{#if data.tagline}
			<p class="mt-4 max-w-xl text-xl text-base-content/70 md:text-2xl">
				{data.tagline}
			</p>
		{/if}
		{#if data.links?.length}
			<div class="mt-8 flex flex-wrap justify-center gap-3">
				{#each data.links as link}
					...
					>
						{#if link.icon}
							<span class="h-5 w-5">{@html getIcon(link.icon)}</span>
						{/if}
						{link.name}
					</a>
				{/each}
