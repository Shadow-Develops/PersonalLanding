<script>
	...
	const icons = {
		...
		twitch: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M2.149 0l-1.612 4.119v16.836h5.731v3.045h3.224l3.045-3.045h4.657l6.269-6.269v-14.686h-21.314zm19.164 13.612l-3.582 3.582h-5.731l-3.045 3.045v-3.045h-4.836v-15.045h17.194v11.463zm-3.582-7.343v6.262h-2.149v-6.262h2.149zm-5.731 0v6.262h-2.149v-6.262h2.149z"/></svg>`,
...
<section id="links" class="px-4 py-16">
		...
		<div class="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4">
			{#each data.items as link}
				<a
					...
					class="group flex flex-col items-center gap-3 rounded-lg border border-base-300 bg-base-100 p-6 transition-all hover:scale-105 hover:border-primary hover:shadow-lg"
				>
					<span class="h-12 w-12 text-primary transition-transform group-hover:scale-110"
						>{@html getIcon(link.icon)}</span
					>
					<span class="text-center text-sm font-medium">{link.name}</span>
				</a>
			{/each}
		</div>
	</div>
